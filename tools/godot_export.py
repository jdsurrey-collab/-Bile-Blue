...
def map_header(name):
    """-> (MAP_CONST, TILESET_NAME, [(dir, target_map), ...])"""
    txt = read(f"data/maps/headers/{name}.asm")
    m = re.search(r"map_header\s+\w+,\s*([A-Z_0-9]+),\s*(\w+)", txt)
    conns = [(c.group(1), c.group(2)) for c in
             re.finditer(r"connection\s+(\w+),\s*(\w+),", txt)]
...
def parse_objects(name):
    ...
    npcs = []
    for a, b, spr, mv, rng, tid in re.findall(
            r"object_event\s+(\d+),\s*(\d+),\s*(\w+),\s*(\w+),\s*(\w+),\s*(\w+)", txt):
        npcs.append(dict(x=int(a), y=int(b), sprite=spr, movement=mv,
                         range=rng, text=tid))
...
def build_map(name, dims):
    ...
    return dict(
        ...
        connections=[{"dir": d, "map": m} for d, m in conns],
