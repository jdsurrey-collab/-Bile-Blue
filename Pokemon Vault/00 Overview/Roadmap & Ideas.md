...
- **[[Godot Port - Progress]] — 1:1 Godot 4.4 port, Phase 6 next.** Phase 5 ("boot → Gary fight ends") is done and verified end-to-end from a cold boot: cultist dream, Pallet Town's Oak intercept, and Oak's Lab's starter choice + real rival battle + permadeath-aware post-battle heal/replacement all work, with zero test shortcuts. Two real bugs found and fixed along the way (not just new-code bugs): a same-frame input cascade that could silently confirm a `ChoiceMenu`'s default option before it was ever visible, and a fresh-save softlock where the front door's `LAST_MAP` warp had nothing recorded to return to. Next: Phase 6, Oak's Lab's post-battle content (parcel delivery from Viridian, the Pokédex gift, Route 22 rival rematch setup) — none of Viridian City/Route 1/Route 22 has been script-wired yet, only Pallet Town/Red's House/Oak's Lab. Full detail in [[Port Plan]]; [[Kanto Walkthrough - Base Game Route]] has the complete area-by-area order to plan the rest of the map-script build queue against.
