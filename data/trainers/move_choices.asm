DEF __move_choices__ = 0

MACRO move_choices
	IF _NARG
		db \# ; all args
	ENDC
	db 0 ; end
	DEF __move_choices__ += 1
ENDM

; move choice modification methods that are applied for each trainer class
TrainerClassMoveChoiceModifications:
	move_choices         ; YOUNGSTER
	move_choices 1       ; BUG_CATCHER
	move_choices 1       ; LASS
	move_choices 1, 3    ; SAILOR
	move_choices 1       ; JR_TRAINER_M
	move_choices 1       ; JR_TRAINER_F
	move_choices 1, 2, 3, 4 ; POKEMANIAC
	move_choices 1, 2, 4    ; SUPER_NERD
	move_choices 1          ; HIKER
	move_choices 1          ; BIKER
	move_choices 1, 3       ; BURGLAR
	move_choices 1          ; ENGINEER
	move_choices 1, 2, 4    ; UNUSED_JUGGLER
	move_choices 1, 3       ; FISHER
	move_choices 1, 3       ; SWIMMER
	move_choices            ; CUE_BALL
	move_choices 1          ; GAMBLER
	move_choices 1, 3       ; BEAUTY
	move_choices 1, 2, 4    ; PSYCHIC_TR
	move_choices 1, 3       ; ROCKER
	move_choices 1          ; JUGGLER
	move_choices 1          ; TAMER
	move_choices 1          ; BIRD_KEEPER
	move_choices 1, 5       ; BLACKBELT
	move_choices 1          ; RIVAL1
	move_choices 1, 3       ; PROF_OAK
	move_choices 1, 2, 4    ; CHIEF
	move_choices 1, 2, 4    ; SCIENTIST
	move_choices 1, 3, 5    ; GIOVANNI
	move_choices 1          ; ROCKET
	move_choices 1, 3, 5    ; COOLTRAINER_M
	move_choices 1, 3, 5    ; COOLTRAINER_F
	move_choices 1, 5       ; BRUNO
	move_choices 1, 5       ; BROCK
	move_choices 1, 3, 5    ; MISTY
	move_choices 1, 3, 5    ; LT_SURGE
	move_choices 1, 3, 5    ; ERIKA
	move_choices 1, 3, 5    ; KOGA
	move_choices 1, 3, 5    ; BLAINE
	move_choices 1, 3, 5    ; SABRINA
	move_choices 1, 2, 4    ; GENTLEMAN
	move_choices 1, 3, 5    ; RIVAL2
	move_choices 1, 3, 5    ; RIVAL3
	move_choices 1, 2, 3, 4, 5 ; LORELEI
	move_choices 1          ; CHANNELER
	move_choices 1, 5       ; AGATHA
	move_choices 1, 3, 5    ; LANCE
	assert __move_choices__ == NUM_TRAINERS, \
		"TrainerClassMoveChoiceModifications: expected {d:NUM_TRAINERS} entries, got {d:__move_choices__}"
